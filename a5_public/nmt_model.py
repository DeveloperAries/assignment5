...
#
    ...
    #     print("8888888",source_padded_chars.size(),target_padded_chars.size())
...
#
    ...
    #     Y = torch.split(Y, 1, dim=0)  # Y此时是元组类型，内部是tensor，维度为（1，b，e）
    #     for Y_t in Y:
    ...
    #
    #     ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
    #     dec_state = self.decoder(Ybar_t, dec_state)  # 单个LSTMcell
    #     dec_hidden, dec_cell = dec_state
    #     # e_t = torch.bmm(enc_hiddens_proj,torch.unsqueeze(dec_hidden,2)).squeeze(2)           #这一步是实现计算e_t，就是那个注意力权值分布 维度是[b,src_len]
    ...
    #
    #     ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
    #     alpha_t = F.softmax(e_t, dim=1)  # 进行softmax，归一化
    #     a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens).squeeze(1)  # 对encoder的hidden输出按注意力权值累加起来，得到[b,2h]
...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        """ Init NMT Model.
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        if self.charDecoder is not None:
            ...
            target_words = target_padded[1:].contiguous().view(-1)
            target_chars = target_padded_chars[1:].view(-1, max_word_len)       #shape is (lenghth ,max_word_len) 这个length是句子的batch*每个句子的长度
            target_outputs = combined_outputs.view(-1, 256)         #每个word-decoder的h输出

            target_chars_oov = target_chars  # torch.index_select(target_chars, dim=0, index=oovIndices)
            rnn_states_oov = target_outputs  # torch.index_select(target_outputs, dim=0, index=oovIndices)  注意看下面那个.t()进行转置了
            oovs_losses = self.charDecoder.train_forward(target_chars_oov.t(),\
                                                         (rnn_states_oov.unsqueeze(0), rnn_states_oov.unsqueeze(0)))
            scores = scores - oovs_losses
