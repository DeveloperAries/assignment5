...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        for i, c in enumerate(self.char_list):
            self.char2id[c] = len(self.char2id)          #将那些字母加入字典中
        self.char_pad = self.char2id['<pad>']             #self.char_pad是index，不是char
        self.char_unk = self.char2id['<unk>']
        self.start_of_word = self.char2id["{"]
        self.end_of_word = self.char2id["}"]
        assert self.start_of_word + 1 == self.end_of_word   #判断单词是否为空
    ...
    def words2charindices(self, sents):
        ...
        """
        #self.char2id.get(c, self.char_unk)字典的get函数，没有此元素则返回self.char_unk，self.char_unk是char_unk的index
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # sents_to_charIndex = self.words2charindices(sents)  pad_sents_char的返回值(batch_size, max_sentence_length, max_word_length)
        padded_sentences_index = pad_sents_char(self.words2charindices(sents),self.char_pad)
        # padded_size = list(padded_sentences_index.size())
        # batch_size = len(padded_sentences_index)
        # max_sentence_length = len(padded_sentences_index[0][0])
        # max_word_length = padded_size[2]
        sents_var = torch.tensor(padded_sentences_index, dtype=torch.long, device=device).permute(1,0,2).contiguous()
        return sents_var
        ### END YOUR CODE
...
if __name__ == '__main__':
    ...
    src_sents = read_corpus(args['--train-src'], source='src')     #read_corpus是从文本中读取，将句子读成单词列表返回
    tgt_sents = read_corpus(args['--train-tgt'], source='tgt')
    # vocab是构建单词表，依据单词频率选取词汇表大小个单词出来
